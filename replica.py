...
class Replica():import zmq
import time
import sys
import StatusMessageimport zmq
import time
import sys
import StatusMessage
import CommitMessage
...
class Replica():
    ...
    def processStatus(self, receivedMessage):
        print("Replica " + str(self.agentID) + " is in processStatus()")
    ...
    def receiveStatus(self, startTime):
        ...
        if(self.queue.checkQueue(self.roundNum) != False):
            while(self.queue.checkQueue(self.roundNum) != False):
        ...
        print("Replica " + str(self.agentID) + " is between check queue and poll")
        while(True):
            timeConsumed = self.getTimeConsumed(startTime)
            if(self.roundTime < timeConsumed):
    ...
    def processPropose(self, receivedMessage):
        print("Replica " + str(self.agentID) + " is in processPropose()")
        ...
        if(receivedProposeMessage.verify() == True and receivedProposeMessage.messageSummary.verify() == True and self.verifySafeValProof(receivedProposeMessage) == True):
            print("Replica " + str(self.agentID) + " receivedProposeMessage.messageSummary.value = " + str(receivedProposeMessage.messageSummary.value))
            ...
            print("Replica " + str(self.agentID) + " inside processPropose() received value " + str(self.receivedProposalValue))
    ...
    def receiveProposal(self, startTime):  
        ...
        print(self.printReplica() + " in receiveProposal(), self.queue.checkQueue(self.roundNum) = " + str(self.queue.checkQueue(self.roundNum)))
        if(self.queue.checkQueue(self.roundNum) != False):
            while(self.queue.checkQueue(self.roundNum) != False):
        ...
        while(True):
            ...
            if(self.receivingSocket.poll((self.roundTime - timeConsumed) * 1000) != 0): # messages are being received
                ...
                print(self.printReplica() + " in receiveProposal() has polled a message")
